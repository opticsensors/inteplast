"""Regression tests for application-owned pilot measurements, with synthetic sources."""
...
from app.ingestion.pilot_3212 import measurements as pilot
...
class Sheet:
    ...
    def cell(self, row, col):
        ...
        return SimpleNamespace(
            value=value,
            ctype=pilot.xlrd.XL_CELL_NUMBER
            if isinstance(value, (int, float))
            else pilot.xlrd.XL_CELL_TEXT,
        )
...
class CorrectionTests(unittest.TestCase):
    ...
    def test_excel_error_code_is_not_a_measurement(self):
        ...
        sheet.cell.return_value = SimpleNamespace(
            ctype=pilot.xlrd.XL_CELL_ERROR, value=7
        )
    ...
    def test_repeated_wrong_bolt_header_uses_cmm_identity_for_both_evaluations(self):
                ...
                rows.append(
                    {
                        ...
                        "tol_inf": -0.1,
                        "tol_sup": 0.0,
                        "medido": 3.97 if idx == 1 else 4.02,
                        "nok": idx == 2,
                    }
                )
        root = ROOT / "synthetic"
        files = [{"muestreo": "03", "cavidad": "c13", "ruta": root / "synthetic.csv"}]
        with (
            patch.object(pilot.cmm, "discover", return_value=files),
            patch.object(
                pilot.cmm, "read_csv", return_value=pilot.cmm.pd.DataFrame(rows)
            ),
        ):
        ...
        for bolt in ("B1", "B2"):
            self.assertEqual(
                records[("03", "c13", "N170", bolt + "-H5.0", 1)]["status"], "inside"
            )
            self.assertEqual(
                records[("03", "c13", "N170", bolt + "-H5.0", 2)]["status"], "outside"
            )

    def test_global_is_separate_from_scan_and_local_points(self):
        keys = [
            pilot.csv_identity({"bloque": b, "idx": 1}, "")
            for b in ("GLOBAL MIN/MAX", "N165 MIN/MAX", "POINT 1 MIN/MAX")
        ]
        self.assertEqual(
            keys, [("N165", "main", 1), ("N165", "scan", 1), ("N165", "P01", 1)]
        )

    def test_chained_prediction_and_missing_after_preserve_uncertainty(self):
        record = {"value": 18.695, "lower": 18.5, "upper": 18.7, "status": "inside"}
        book = Mock()
        book.sheet_by_name.return_value = Sheet()
        comparison = pilot.compare(
            {("03", "c13", "N240", "main", 1): record},
            {"2": book},
            ROOT,
            "N240",
            "main",
        ...
        self.assertEqual([s["delta"] for s in item["steps"]], [0.02, -0.08])
        self.assertIsNone(item["after"])
        self.assertTrue(
            any("falta la medición posterior" in w for w in comparison["warnings"])
        )
